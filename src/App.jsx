...
const AdminLogin = ({ onSuccess, onBack }) => {
  ...
  const handleDigit = (d) => {
    if (pin.length >= ADMIN_PIN.length) return;
    setPin(pin + d);
    setError(false);
  };
...
// ─── Admin Page ───
const AdminPage = ({ gameState, players, history, onBack, onTogglePaid, onRemovePlayer, onDrawNumbers, onReset, onNextWeek }) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, #0a0e1a 0%, #1a1040 40%, #0d2137 100%)", fontFamily: "'Segoe UI', system-ui, sans-serif", color: "#e8e6f0", padding: "20px 16px 60px" }}>
      ...
      {players.length > 0 && gameState.phase === "entry" && (
        <button onClick={onDrawNumbers} style={{
          width: "100%", padding: "18px", borderRadius: 16, border: "2px solid #FFD700", cursor: "pointer", fontSize: 18, fontWeight: 800, letterSpacing: 1,
          background: "linear-gradient(135deg, rgba(255,215,0,0.15), rgba(255,165,0,0.1))", color: "#FFD700", textTransform: "uppercase", marginBottom: 24,
        }}>🎱 Draw This Week's Numbers</button>
      )}

      {gameState.phase === "results" && (
        <button onClick={onNextWeek} style={{
          width: "100%", padding: "18px", borderRadius: 16, border: "none", cursor: "pointer", fontSize: 16, fontWeight: 800, letterSpacing: 1,
          background: "linear-gradient(135deg, #6366f1, #8b5cf6)", color: "#fff", textTransform: "uppercase", marginBottom: 24,
        }}>Start Week #{gameState.week_number + 1} →</button>
      )}
      ...
      <button onClick={onReset} style={{ display: "block", margin: "30px auto 0", padding: "8px 20px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.08)", background: "transparent", color: "#555", fontSize: 12, cursor: "pointer" }}>Reset Everything</button>
    </div>
  );
};
...
export default function App() {
  ...
  if (page === "admin") return <AdminPage gameState={gameState} players={players} history={history} onBack={() => setPage("main")} onTogglePaid={togglePaid} onRemovePlayer={removePlayer} onDrawNumbers={drawNumbers} onReset={resetAll} onNextWeek={nextWeek} />;
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, #0a0e1a 0%, #1a1040 40%, #0d2137 100%)", fontFamily: "'Segoe UI', system-ui, sans-serif", color: "#e8e6f0", padding: "20px 16px 60px", position: "relative", overflow: "hidden" }}>
      ...
      {gameState.phase === "results" && (
        <div>
          ...
          <div style={{ textAlign: "center", marginTop: 20, padding: "14px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ fontSize: 13, color: "#8b86ad" }}>Admin must start the next week</div>
            <button onClick={() => setPage("adminLogin")} style={{ marginTop: 8, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "8px 20px", color: "#6b6890", cursor: "pointer", fontSize: 12, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase" }}>🔒 Admin Login</button>
          </div>
        </div>
      )}
